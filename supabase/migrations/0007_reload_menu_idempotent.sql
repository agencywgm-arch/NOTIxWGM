-- ============================================================================
--  NOTI Calling — 0007_reload_menu_idempotent.sql
--
--  Rend seed_noti_menu() rejouable à volonté (bouton « Recharger la carte
--  Noti Club » dans l'onglet Carte côté app) : sans ce patch, chaque appel
--  dupliquait tous les articles. Les articles déjà présents sont désormais
--  mis à jour (prix, description, variantes...) au lieu d'être recréés ; le
--  statut « épuisé » / « retiré » choisi par le staff n'est jamais écrasé.
--
--  À exécuter une seule fois, après 0001-0005 (et 0006 si déjà appliqué).
...
--
--  ⚠️ Si la carte a déjà été rechargée manuellement plusieurs fois avant ce
--  patch (donc avec des doublons existants), l'index unique ci-dessous
--  échouera à la création. Dans ce cas, nettoyez d'abord les doublons :
--    delete from public.products a using public.products b
--    where a.venue_id = b.venue_id and a.universe = b.universe and a.name = b.name
--      and a.ctid < b.ctid;
-- ============================================================================
